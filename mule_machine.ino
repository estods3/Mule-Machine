...
// PINOUT DEFINITION
const int ledGreen_Lime =  4;
const int ledYellow_GingerBeer = 5;
const int ledOrange_Vodka =  6;

const int pump_Lime =  9;
const int pump_GingerBeer = 10;
const int pump_Vodka = 11;
...
const double calibration_ratio_Lime = 0.2; // 0.0625 Cups per serving --> 0.5/0.75 % of calibration_time_pour_duration spent pouring lime juice
const double calibration_ratio_GingerBeer = 0.6; // 0.5 Cups per serving --> 0.5/0.75 % of calibration_time_pour_duration spent pouring ginger beer
...
void setup() {
  ...
  pinMode(lever_buttonPin, INPUT);
  pinMode(cupSensor_buttonPin, INPUT);
...
void loop() {
  ...
  lever_is_pulled = digitalRead(lever_buttonPin) == HIGH; //&& CUP_present && numDrinksTilEmpty > 0
  if (lever_is_pulled) {
       ...
       while(lever_is_pulled){
           ...
           delay(calibration_ratio_Vodka * calibration_time_pour_duration);
           digitalWrite(pump_Vodka, LOW);
           digitalWrite(ledOrange_Vodka, LOW);

           delay((calibration_ratio_Lime * calibration_time_pour_duration) - (calibration_ratio_Vodka * calibration_time_pour_duration));
           digitalWrite(pump_Lime, LOW);
           digitalWrite(ledGreen_Lime, LOW);

           delay((calibration_ratio_GingerBeer * calibration_time_pour_duration) - (calibration_ratio_Lime * calibration_time_pour_duration) - (calibration_ratio_Vodka * calibration_time_pour_duration));
           digitalWrite(pump_GingerBeer, LOW);
           digitalWrite(ledYellow_GingerBeer, LOW);
                     
           lever_is_pulled = digitalRead(lever_buttonPin) == HIGH;
       }
       digitalWrite(ledOrange_Vodka, LOW);
       digitalWrite(ledYellow_GingerBeer, LOW);
       digitalWrite(ledGreen_Lime, LOW);
